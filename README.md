...
### Класс ContactsForm
Работа с формой, содержащей контаутную информацию покупателя (телефон и электронная почта)
#### Поля:
- `emailElement` - элемент ввода электронной почты
- `phoneElement` - элемент ввода номера телефона
#### Конструктор
`constructor(container: HTMLFormElement, events: IEvents)` - принимает HTML-форму, с которой будет работать класс, и экземпляр IEvents для обработки событий. Вызывает конструктор родительского класса Form с переданными параметрами.
#### Сеттеры
- `email` установка значения элемента ввода электронной почты
- `phone` - установка значения элемента ввода номера телефона
